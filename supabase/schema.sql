...
-- Opakovaný výdaj (Pro): šablona, ze které RPC `run_due_recurring` vytváří
-- řádky v `expenses`, když dozraje `next_run`. Vždy rovným dílem (equal).
create table if not exists public.recurring_expenses (
  ...
  description    text not null,
  category       text not null default 'other',
  -- weekly = každých N týdnů · monthly = každých N měsíců na anchor_day
  ...
  anchor_day     int check (anchor_day between 1 and 31),   -- jen 'monthly', jinak NULL
...
-- Vazba vygenerovaného výdaje na šablonu; u ručních výdajů NULL.
-- `on delete set null`: smazání šablony nesmí smazat výdaje, co drží bilance.
...
-- Opakované výdaje: vytvoří řádky v `expenses` ze všech šablon, kterým
-- dozrál `next_run`. Volá klient při otevření skupiny. Vrací počet
-- vytvořených výdajů. Idempotentní přes posun `next_run`; `skip locked`
-- ošetří dvojí souběžný běh.
...
begin
  ...
  loop
    ...
    -- výdaj, zbytek jen posune `next_run` dopředu (strop, ať dlouhá pauza
    -- nezaplaví skupinu). Posun data je pořád stejný blok, takže
    -- `anchor_day` se u monthly aplikuje i ve skip-ahead části.
...
-- Jen vlastní profil. `using (true)` platilo i pro `anon`, takže kdokoli
-- s veřejným anon klíčem si přečetl e-maily a jména všech uživatelů.
-- Aplikace nikde nečte cizí profil — jména členů drží `group_members`,
-- `is_pro` se dotýká jen UI plátce. (audit 2026-08-27, S1)
...
-- Vkládat členy smí buď zakladatel skupiny, nebo někdo, kdo v ní už je.
-- Dřívější větev `or user_id = auth.uid()` dovolovala přidat se do libovolné
-- skupiny podle jejího UUID a obejít připojení kódem. Připojení přes kód
-- jde přes SECURITY DEFINER RPC `join_group_choose`, které RLS obchází.
-- (audit 2026-08-27, S2)
...
-- Opakované výdaje: stejný model jako `expenses` — vše jen členům skupiny.
...
create policy "recurring_select" on public.recurring_expenses for select using (public.is_group_member(group_id));
create policy "recurring_insert" on public.recurring_expenses for insert with check (public.is_group_member(group_id));
